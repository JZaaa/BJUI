...
+(function($) {
  ...
  Datagrid.prototype.TOOLS = function() {
    ...
    var tools = {
      ...
      getPageInterval: function(count, pageCurrent, showPageNum) {
        var half = Math.ceil(showPageNum / 2); var limit = count - showPageNum

        var start = pageCurrent > half ? Math.max(Math.min(pageCurrent - half, limit), 0) : 0

        var end = pageCurrent > half ? Math.min((pageCurrent + half), count) : Math.min(showPageNum, count)
      ...
      getRight: function($obj) {
        var width = 0; var index = $obj.data('index'); var $firstTds = that.$tbody.find('> tr:first > td:lt(' + (index + 1) + ')')

        if (!$firstTds || !$firstTds.length) $firstTds = that.$colgroupH.find('> col:lt(' + (index + 1) + ')')
        $firstTds.each(function() {
          var $td = $(this); var w = $td.is(':hidden') ? 0 : $td.outerWidth()
      ...
      getRight4Lock: function(index) {
        var width = 0; var $td = that.$lockTbody.find('> tr:first > td:eq(' + index + ')'); var $firstTds = $td && $td.prevAll().add($td)
      ...
      columns2Arr: function(columns, rowArr, index, parent) {
        ...
        $.each(columns, function(i, n) {
          var len = rowArr[index].length; var colspan
      ...
      initTbody: function(data, refreshFlag) {
        var tools = this; var allData = that.allData; var type = options.dataType || 'json'; var model = that.columnModel; var hiddenFields = that.hiddenFields; var regional = that.regional; var newData = []; var attach = that.attach; var json
        var paging = that.paging; var start = 0; var end = paging.pageSize
        var doInit = function() {
          ...
          if (type == 'array' && data && data.length && $.type(data[0]) == 'array') {
            var a1 = start; var a2 = end; var arrData = []; var _index
      ...
      createTrs: function(datas, refreshFlag) {
        var tools = this; var model = that.columnModel; var paging = that.paging; var editFrag = BJUI.doRegional(FRAG.gridEditBtn, that.regional); var lockedCols = []
        ...
        $.each(datas, function(i, trData) {
          var $tr = $('<tr></tr>'); var $td; var linenumber = options.linenumberAll ? ((paging.pageCurrent - 1) * paging.pageSize + (i + 1)) : (i + 1)

          $.extend(trData, { gridNumber: linenumber, gridIndex: i })

          $.each(model, function(j, n) {
            var name = n.name || 'datagrid-noname'; var label = trData[name]; var align = ''; var cls = ''; var _label
            ...
            if (n.render && typeof n.render === 'function') {
              var render_label = ''; var render_items = false
      ...
      loadData: function(data, refreshFlag) {
        var tools = this; var url = options.dataUrl; var dataType = options.dataType || 'json'; var model = that.columnModel
          ...
          if (dataType == 'xml') {
            ...
            $(response).find('row').each(function() {
              ...
              $(this).find('cell').each(function(i) {
                var $cell = $(this); var label = $cell.text(); var name = $cell.attr('name')
      ...
      createThead: function() {
        var tools = this; var columns = options.columns; var rowArr = []; var rows = []; var label; var align; var width; var colspan; var rowspan; var resize; var menu
        ...
        $.each(rowArr, function(i, arr) {
          var $tr = $('<tr style="height:25px;"></tr>'); var $num = '<th class="datagrid-number"></th>'; var $th
      ...
      createTbody: function() {
        var tools = this; var data = options.data; var model = that.columnModel; var cols = []
      ...
      setBoxbH: function(height) {
        var boxH = height || options.height; var topM = 0; var h
        ...
        if (that.$box_paging) { boxH -= that.$box_paging.outerHeight() }
        if (that.$boxF) { boxH -= that.$boxF.outerHeight() }
      ...
      showSubMenu: function($li, $menu, $submenu) {
        var left; var width = $menu.outerWidth(); var submenu_width = $submenu.data('width') || $submenu.outerWidth(); var animate_op; var boxWidth = that.$boxH.width()
      ...
      locking: function($th) {
        var index = $th.data('index'); var columnModel = that.columnModel[index]; var lockFlag = columnModel.lock; var locked = columnModel.locked; var $menu = that.$menu; var $ul = $menu.find('> ul'); var $lockli = $ul.find('> li.' + that.classnames.li_lock); var $unlockli = $lockli.next()
          ...
          $.each(that.columnModel, function(i, n) {
            ...
            var colClick = function(n) {
              $col.click(function() {
                ...
                var $this = $(this); var check = !$this.find('i').hasClass('fa-check-square-o'); var index = n.index
      ...
      showhide: function(model, showFlag) {
        var index = model.index; var $th = model.th; var $trs = that.$tbody.find('> tr'); var display = showFlag ? '' : 'none'
      ...
      jumpPage: function(pageCurrent, pageSize) {
        var allData = that.allData; var filterDatas
      ...
      quickSort: function(model) {
        ...
        var $th = model.th; var data = that.data; var allData = that.allData; var postData; var direction; var name = model.name; var type = model.type; var $ths = that.$thead.find('> tr > th.datagrid-quicksort-th')

        if (!name) name = 'datagrid-noname'

        var sortData = function(data) {
          data.sort(function(a, b) {
            var typeA = (typeof a[name]); var typeB = (typeof b[name])
      ...
      quickFilter: function(model, filterDatas) {
        ...
        var tools = this; var $th = model.th; var data = that.data; var allData = that.allData; var name = model.name; var postData; var fDatas
      ...
      getRemoteFilterData: function(isPaging) {
        ...
        if (that.filterDatas && !$.isEmptyObject(that.filterDatas)) {
          $.each(that.filterDatas, function(name, v) {
            ...
            if (options.jsonPrefix) { name = options.jsonPrefix + '.' + name }

            if (v.datas.andor) { filterDatas.push({ name: 'andor', value: v.datas.andor }) }
      ...
      setDomData: function(tr) {
        var columnModel = that.columnModel; var data = {}; var hideDatas = tr.attr('data-hidden-datas'); var attach = that.attach

        tr.find('> td').each(function(i) {
          var $td = $(this); var model = columnModel[i]; var val = $td.attr('data-val') || $td.text()
      ...
      initEditInputs: function() {
        ...
        $.each(columnModel, function(i, op) {
          var name = op.name; var rule = ''; var pattern = ''; var selectoptions = []; var attrs = ''
      ...
      contextmenuH: function() {
        ...
        that.$tableH.on('contextmenu', function(e) {
          ...
          $(document).on('click', function(e) {
            ...
            if (!$showhide.length) { that.$showhide.css({ left: '50%', top: 0, opacity: 0.2, 'z-index': '' }).hide().appendTo(that.$grid) }
          })
  ...
  Datagrid.prototype.createTable = function() {
    var that = this; var options = that.options; var tools = that.tools
  ...
  Datagrid.prototype.setColumnModel = function() {
    var that = this; var options = that.options; var $trs = that.$thead.find('> tr'); var rows = []; var ths = []; var trLen = $trs.length

    if (!that.isDom) {
      that.tools.appendColumns()

      var $th; var _rowspan = trLen > 1 ? ' rowspan="' + trLen + '"' : ''
    ...
    if ($trs.length && trLen == 1) {
      ...
      $trs.find('> th').each(function(i) {
        var $th = $(this).addClass('single-row'); var op = $th.data('options'); var oW = $th.attr('width') || $th.outerWidth(); var label = $th.html()
      ...
      $trs.each(function(len) {
        var next_rows = []; var next_ths = []; var index = -1; var next_index = 0
        ...
        $(this).height(25).find('> th').each(function(i) {
          var $th = $(this); var op = $th.data('options') || $th.data('datagrid.column') || {}; var colspan = parseInt($th.attr('colspan') || 0); var rowspan = parseInt($th.attr('rowspan') || 0); var oW = $th.attr('width') || $th.outerWidth(); var label = $th.html()
  ...
  Datagrid.prototype.init = function() {
    ...
    var that = this; var options; var tools = that.tools; var $parent = that.$element.parent(); var boxWidth
  ...
  Datagrid.prototype.refresh = function() {
    var that = this; var options = that.options; var tools = that.tools; var isDom = that.isDom; var pageInfo = BJUI.pageInfo; var paging = that.paging; var postData = {}
  ...
  Datagrid.prototype.destroy = function() {
    var that = this; var $element = that.$grid.data('bjui.datagrid.table')
  ...
  Datagrid.prototype.initTop = function() {
    var that = this; var options = that.options; var regional = that.regional; var hastoolbaritem = false; var $group; var groupHtml = '<div class="btn-group" role="group"></div>'; var btnHtml = '<button type="button" class="btn" data-icon=""></button>'
    ...
    if (options.showToolbar) {
      if (options.toolbarItem || options.toolbarCustom) {
              ...
              that.$toolbar_add = $(btnHtml).attr('data-icon', 'sign-in').addClass('btn-blue').text(BJUI.getRegional('datagrid.import'))
                ...
                .on('click', function(e) {
                  if (options.importOption) {
                    ...
                    if (typeof opts === 'string') { opts = opts.toObj() }
        ...
        if (options.toolbarCustom) {
          var $custom; var $custombox = $('<div style="display:inline-block;"></div>')
  ...
  Datagrid.prototype.initThead = function() {
    var that = this; var options = that.options; var tools = that.tools; var columnModel = that.columnModel; var width; var cols = []; var $firstTds = that.$tbody.find('> tr:first-child > td')
    ...
    $ths.filter('.datagrid-quicksort-th')
      .on('click.bjui.datagrid.quicksort', function(e) {
        ...
        if (!$target.closest('.' + that.classnames.th_cell).length && !that.isResize) { tools.quickSort(columnModel[$(this).data('index')]) }
      })

    // events - filter
    $ths.filter('.datagrid-column-menu')
      .on('bjui.datagrid.th.filter', function(e, flag) {
        var $th = $(this); var $btn = $th.find('> div > .' + that.classnames.th_cell + '> .' + that.classnames.btn_menu + '> .btn')
  ...
  Datagrid.prototype.fixedWidth = function(isInit) {
    var that = this; var options = that.options; var bW; var excludeW = 0; var fixedW; var width; var columnModel = that.columnModel; var length = columnModel.length
    ...
    var setNewWidth = function() {
      ...
      if (bW) {
        ...
        if (that.table_width > bW) { that.$pagingCon && that.$pagingCon.width(bW) } else { that.$pagingCon && that.$pagingCon.width(that.table_width) }
      }
    }
  ...
  Datagrid.prototype.initTbody = function() {
    var that = this; var options = that.options; var tools = that.tools; var $trs = that.$tbody.find('> tr'); var $tds = $trs.find('> td'); var width = '0'
  ...
  Datagrid.prototype.initEvents = function($trs) {
    var that = this; var options = that.options; var trs = that.$tbody.find('> tr')

    if (!$trs) $trs = trs
    $trs
      .on('click.bjui.datagrid.tr', function(e, checkbox) {
        var $tr = $(this); var index = $tr.index(); var $selectedTrs = that.$tbody.find('> tr.' + that.classnames.tr_selected); var $last = that.$lastSelect; var checked; var $lockTrs = that.$lockTbody && that.$lockTbody.find('> tr')
      ...
      .on('mouseenter.bjui.datagrid', function(e) {
        var $tr = $(this); var index = $tr.index()
    ...
    // custom event - delete
      .on('bjui.datagrid.tr.delete', function(e) {
        var $tr = $(this); var index = $tr.index(); var data = that.data; var gridIndex; var allData = that.allData; var $lockTrs = that.$lockTbody && that.$lockTbody.find('> tr')
        ...
        if (that.columnModel[0] == that.linenumberColumn) {
          $tr.nextAll().each(function() {
            var $td = $(this).find('> td:eq(0)'); var num = parseInt($td.text())
    ...
    if (that.checkboxColumn && that.checkboxColumn.th) {
      that.checkboxColumn.th.find('input').on('ifChanged', function() {
        var checked = $(this).is(':checked'); var $trs = that.$tbody.find('> tr')
  ...
  Datagrid.prototype.initTfoot = function() {
    var that = this; var options = that.options; var tools = that.tools; var columnModel = that.columnModel; var $tr = $('<tr></tr>')
  ...
  Datagrid.prototype.selectedRows = function(rows, selected) {
    var that = this; var $lockTrs = that.$lockTbody && that.$lockTbody.find('> tr'); var $trs = that.$tbody.find('> tr'); var n
    ...
    if (rows == null) {
      ...
      rows.each(function() {
        var $row = $(this); var index = $row.index()
  ...
  Datagrid.prototype.initLock = function() {
    var that = this; var columnModel = that.columnModel
  ...
  Datagrid.prototype.colLock = function(column, lockFlag) {
    var that = this; var $th; var index; var columnModel
  ...
  Datagrid.prototype.fixedLockItem = function(type) {
    var that = this; var columnModel = that.columnModel; var $filterThs = that.$thead.find('> tr.datagrid-filter > th'); var $lockTrs = that.$lockTbody && that.$lockTbody.find('> tr')
    ...
    if (!type) {
      ...
      if ($filterThs && $filterThs.length) {
        that.$lockThead && that.$lockThead.find('> tr.datagrid-filter > th:visible').each(function() {
          var $lockTh = $(this); var index = $lockTh.index(); var $th = $filterThs.eq(index)
      ...
      if ($lockTrs && $lockTrs.length) {
        $lockTrs.filter('.' + that.classnames.tr_edit).each(function() {
          var $lockTr = $(this); var $lockTd = $lockTr.find('> td:eq(' + columnModel.lockIndex + ')'); var tr_index = $lockTr.index(); var $tr; var $td
      ...
      if ($lockTrs && $lockTrs.length) {
        $lockTrs.filter('.' + that.classnames.tr_edit).each(function() {
          var $lockTr = $(this); var tr_index = $lockTr.index()

          $lockTr.find('> td.' + that.classnames.td_edit + ':visible').each(function() {
            var $lockTd = $(this); var td_index = $lockTd.index(); var model = columnModel[td_index]; var $tr; var $td
  ...
  Datagrid.prototype.doLock = function() {
    var that = this; var options = that.options; var tools = that.tools; var columnModel = that.columnModel; var tableW = that.$tableH.width(); var width = 0; var $trs; var $lockTrs; var lockedLen = 0
    var hasFoot = that.$boxF && true; var top = 0
    ...
    var lockH = that.$lockTableH.height(); var H = that.$thead.height(); var lockFH = that.$lockTableF && that.$lockTableF.height(); var HF = that.$tableF && that.$tableF.height()
    ...
    setTimeout(function() {
      var lockBH = that.$lockTableB.height(); var HB = that.$tableB.height()

      if (lockBH != HB) {
        if (lockBH > HB) {
          $lockTrs.each(function(tr_index) {
            var $lockTr = $(this); var $lockTd = $lockTr.find('> td:visible:first'); var newH = $lockTd.outerHeight()
          ...
          $trs.each(function(tr_index) {
            var $tr = $(this); var $td = $tr.find('> td:visible:first'); var newH = $td.outerHeight()
  ...
  Datagrid.prototype.initLockEvents = function($locktrs) {
    ...
    $locktrs
      ...
      .on('ifChanged', function() {
        var $this = $(this); var $tr = $this.closest('tr'); var index = $tr.index()
  ...
  Datagrid.prototype.filterInThead = function() {
    var that = this; var options = that.options; var tools = that.tools; var regional = that.regional; var columnModel = that.columnModel; var filterData = {}
    ...
    $.each(columnModel, function(i, n) {
      var $input = $(that.inputs[i]); var $th = $('<th></th>')
  ...
  Datagrid.prototype.showLinenumber = function(flag) {
    var that = this; var options = that.options; var model = that.columnModel; var numberModel = model[0]; var data = that.data

    if (numberModel == that.linenumberColumn) {
      if (typeof flag === 'string' && (flag == 'lock' || flag == 'unlock')) {
        that.colLock(numberModel.th, flag == 'lock')
      } else {
        that.showhideColumn(numberModel.th, !!flag)
      }
    } else if (flag) {
      var $trsH = that.$thead.find('> tr'); var col = '<col style="width:30px;">'; var $th; var $filterTr = $trsH.filter('.datagrid-filter'); var rowspan = $trsH.length - $filterTr.length
  ...
  Datagrid.prototype.showCheckboxcol = function(flag) {
    var that = this; var options = that.options; var model = that.columnModel; var numberModel = model[0]; var checkModel; var data = that.data
    ...
    if (checkModel) {
      if (typeof flag === 'string' && (flag == 'lock' || flag == 'unlock')) {
        that.colLock(checkModel.th, flag == 'lock')
      } else {
        that.showhideColumn(checkModel.th, flag)
      }
    } else if (flag) {
      var $trsH = that.$thead.find('> tr'); var col = '<col style="width:26px;">'; var $th; var $td; var $filterTr = $trsH.filter('.datagrid-filter'); var rowspan = $trsH.length - $filterTr.length
  ...
  Datagrid.prototype.showEditCol = function(flag) {
    var that = this; var options = that.options; var model = that.columnModel; var editModel = model[model.length - 1]; var data = that.data

    if (editModel == that.editBtnsColumn) {
      that.showhideColumn(editModel.th, !!flag)
    } else if (flag) {
      var $trsH = that.$thead.find('> tr'); var col = '<col style="width:110px;">'; var $th; var $td; var $filterTr = $trsH.filter('.datagrid-filter'); var rowspan = $trsH.length - $filterTr.length
  ...
  Datagrid.prototype.colResize = function() {
    ...
    var $thead = that.$thead

    var $resizeMark = that.$grid.find('> .resizeProxy')
    ...
    $thead.find('> tr > th').each(function(i) {
      var $th = $(this); var $resize = $th.find('> div > .' + that.classnames.th_cell + '> .' + that.classnames.th_resizemark)

      $resize.on('mousedown.bjui.datagrid.resize', function(e) {
        var ofLeft = that.$boxH.scrollLeft(); var marginLeft = parseInt(that.$boxH.css('marginLeft') || 0); var left = tools.getRight($th) - ofLeft + marginLeft; var index = $th.data('index'); var model = columnModel[index]; var width = model.th.width()

        var $trs = that.$tbody.find('> tr'); var $lockTrs = that.$lockTbody && that.$lockTbody.find('> tr'); var lockH = that.$lockTableB && that.$lockTableB.height(); var H = that.$tableB.height(); var lockH_new; var H_new
        ...
        $resizeMark
          ...
          .basedrag({
            ...
            stop: function() {
              ...
              var move = new_left - left

              var newWidth = move + width

              var tableW = that.$tableH.width() + move

              var lockW = that.$lockTableH && that.$lockTableH.width() + move
              ...
              setTimeout(function() {
                ...
                if (that.$lockTbody) {
                  ...
                  if (lockH_new != lockH || H_new != H) {
                    if (lockH_new > H_new) {
                      $lockTrs.each(function(tr_index) {
                        var $lockTr = $(this); var $lockTd = $lockTr.find('> td:eq(' + model.lockIndex + ')'); var newH = $lockTd.outerHeight()
                      ...
                      $trs.each(function(tr_index) {
                        var $tr = $(this); var $td = $tr.find('> td:eq(' + index + ')'); var newH = $td.outerHeight()
  ...
  Datagrid.prototype.colMenu = function() {
    var that = this; var options = that.options; var tools = that.tools; var regional = that.regional; var $ths = that.$trsH.find('> th.' + that.classnames.th_menu); var $menu = that.$grid.find('> .' + that.classnames.s_menu)
    ...
    $ths.each(function() {
      ...
      var model = that.columnModel[index]

      var $cell = $th.find('> div > .' + that.classnames.th_cell)

      var $btnBox = $cell.find('> .' + that.classnames.btn_menu)
      ...
      if (!$btnBox.length) {
        ...
        $btn.click(function() {
          var $tr = $th.closest('tr'); var rowspan = parseInt($th.attr('rowspan') || 1); var left = $(this).offset().left - that.$grid.offset().left - 1; var top = (that.$trsH.length - rowspan) * 25 + (13 * rowspan) + 11; var $showhide = $menu.find('> ul > li.datagrid-li-showhide'); var menu_width; var submenu_width
    ...
    that.$grid.on('click.bjui.datagrid.filter', function(e) {
      var $target = $(e.target); var $menu = that.$grid.find('.' + that.classnames.s_menu + ':visible')
  ...
  Datagrid.prototype.colShowhide = function(showFlag) {
    var that = this; var options = that.options; var tools = that.tools; var $menu = that.$menu; var $ul = $menu.find('> ul'); var $showhideli = $ul.find('> li.' + that.classnames.li_showhide)
  ...
  Datagrid.prototype.showhideColumn = function(column, showFlag) {
    var that = this; var tools = that.tools; var index; var model
  ...
  Datagrid.prototype.colFilter = function($th, filterFlag) {
    var that = this; var options = that.options; var tools = that.tools; var regional = that.regional; var $filter = $th.data('bjui.datagrid.filter'); var $menu = that.$menu; var $filterli = $menu.find('> ul > li.' + that.classnames.li_filter)
    ...
    if (filterFlag) {
      ...
      if (!$filter || !$filter.length) {
        ...
        var index = $th.data('index'); var model = that.columnModel[index]; var type = model.type || 'string'; var operator = model.operator || []

        var $filterA = $filter.find('span.filter-a')

        var $filterB = $filter.find('span.filter-b')

        var $select = $('<select data-toggle="selectpicker" data-container="true"></select>')

        var $input = $(that.inputs[index])
        ...
        var $clear = $filter.find('button.clear')

        var $selects = $filter.find('select')

        var $selA = $selects.first()

        var $selB = $selects.last()

        var $andOr = $selects.eq(1)

        $ok.click(function() {
          var operatorA = $selA.val(); var valA = $valA.val().trim(); var operatorB = $selB.val(); var valB = $valB.val().trim(); var andor = $andOr.val()
          var filterDatas = {}
      ...
      $menu.find('> ul > li:not(".' + that.classnames.li_filter + '")').on('mouseover', function() {
        if ($filterli.hasClass('active')) { $filterli.trigger('bjui.datagrid.th.submenu.hide', [$menu, $filter]) }
      })
    } else {
      $filterli.hide()
    }
  ...
  Datagrid.prototype.initPaging = function() {
    var that = this; var tools = that.tools; var options = that.options; var paging = that.paging; var pr = BJUI.regional.pagination; var btnpaging = FRAG.gridPaging; var pageNums = []; var pageCount = paging.pageCount; var interval; var selectPages = []
    ...
    var $first = $jumpto.next()

    var $prev = $first.next()

    var $next = that.$btnpaging.find('> li.page-next')
    ...
    that.$paging.on('click', '.page-num', function(e) {
      ...
      var pageCurrent = that.paging.pageCurrent; var interval = tools.getPageInterval(that.paging.pageCount, pageCurrent, paging.showPagenum); var pageNums = []
    ...
    $first.on('click', function() {
      if (that.paging.pageCurrent > 1) { that.jumpPage(1) }
    })

    $prev.on('click', function() {
      if (that.paging.pageCurrent > 1) { that.jumpPage(that.paging.pageCurrent - 1) }
    })

    $next.on('click', function() {
      if (that.paging.pageCurrent < that.paging.pageCount) { that.jumpPage(that.paging.pageCurrent + 1) }
    })

    $last.on('click', function() {
      if (that.paging.pageCurrent < that.paging.pageCount) { that.jumpPage(that.paging.pageCount) }
    })
  ...
  Datagrid.prototype.jumpPage = function(pageCurrent, pageSize) {
    var that = this; var paging = that.paging; var pageCount = paging.pageCount
  ...
  Datagrid.prototype.add = function(addLocation) {
    ...
    var that = this; var options = that.options; var tools = that.tools; var paging = that.paging; var trs; var obj = {}; var data = []; var addObj; var startNumber = (paging.pageCurrent - 1) * paging.pageSize; var linenumber

    var addTr = function(linenumber) {
      var $tr = $('<tr></tr>').addClass(that.classnames.tr_add); var $lockTr = $tr.clone()

      $.each(that.columnModel, function(i, n) {
        var label = ''; var $td = $('<td></td>')

        if (n.gridNumber) { $td.addClass(that.classnames.td_linenumber).text(0) } else if (n.gridCheckbox) { $td.addClass(that.classnames.td_checkbox).html('<div><input type="checkbox" data-toggle="icheck" name="datagrid.checkbox" value="true"></div>') } else if (n.gridEdit) { $td.addClass(that.classnames.s_edit).data('isAdd', true).html(BJUI.doRegional(FRAG.gridEditBtn, that.regional)) } else $td.text('')
  ...
  Datagrid.prototype.edit = function($trs) {
    var that = this; var options = that.options; var tools = that.tools; var type = options.editMode; var columnModel = that.columnModel; var $editTd
    ...
    $editTd.each(function() {
      var $td = $(this); var $btns = $td.find('button'); var $edit = $btns.first(); var $update = $edit.next(); var $save = $update.next(); var $cancel = $save.next(); var $delete = $cancel.next()

      $edit.on('click', function(e, data) {
        var $btn = $(this); var $tr = $btn.closest('tr'); var isAdd = $td.data('isAdd')
  ...
  Datagrid.prototype.doEditRow = function(rows, type, isAdd) {
    var that = this; var $trs; var $editBtn; var datas = []
    ...
    if (typeof rows === 'object') {
      ...
      var $myTrs = that.$tbody.find('> tr'); var $editTrs; var rows = rows.split(',')
    ...
    $trs.each(function() {
      var $tr = $(this); var tr_index = $tr.index(); var data
  ...
  Datagrid.prototype.doCancelEditRow = function(row) {
    ...
    $trs.each(function() {
      var $tr = $(this); var $cancelBtn = $tr.find('> td.' + that.classnames.s_edit + ' > button.cancel')
  ...
  Datagrid.prototype.doSaveEditRow = function(row) {
    var that = this; var options = that.options; var $tr
    ...
    if ($tr.length == 1) {
      if ($tr.hasClass(that.classnames.tr_edit)) { this.updateEdit($tr) }
    } else {
      if (options.saveAll) {
        that.saveAll($tr)
      } else {
  ...
  Datagrid.prototype.delRows = function(rows) {
    var that = this; var options = that.options; var beforeDelete = options.beforeDelete; var confirmMsg = options.delConfirm; var $trs
    ...
    if (typeof rows === 'object') {
      ...
      var $myTrs = that.$tbody.find('> tr'); var $delTrs; var rows = rows.split(',')
    ...
    var doDel = function() {
      ...
      // remote delete
      if (options.delUrl) {
        var postData = []; var callback = options.delCallback

        $trs.each(function() {
          var $tr = $(this); var index = $tr.index(); var data; var delData
  ...
  Datagrid.prototype.inlineEdit = function($tr, isAdd, data) {
    ...
    var that = this; var options = that.options; var tools = that.tools; var columnModel = that.columnModel; var $tds = $tr.find('> td'); var tds_length = $tds.length; var tr_index = $tr.index()
    ...
    $tds.each(function(i) {
      var $td = $(this); var op = columnModel[i]; var val = op && op.name && data[op.name]; var html = $td.html(); var input = that.inputs[i]; var $input
  ...
  Datagrid.prototype.saveAll = function($trs) {
    var that = this; var options = that.options; var callback = options.editCallback; var $tr; var data; var changeData; var tempData; var postData = []; var len = $trs && $trs.length
    ...
    if (!len) {
      ...
      if (callback) {
        ...
        callback = function(json) {
          var complete = false; var returnData = []
          ...
          if (complete) {
            $trs.each(function(i) {
              var $tr = $(this); var data = that.isDom ? $tr.data('initData') : that.data[$tr.index()]
  ...
  Datagrid.prototype.updateEdit = function($tr, $btn) {
    var that = this; var options = that.options; var callback = options.editCallback; var data; var changeData; var tempData; var postData = []
  ...
  Datagrid.prototype.cancelEdit = function($trs) {
    ...
    $trs.each(function() {
      var $tr = $(this); var tr_index = $tr.index()

      if ($tr.hasClass(that.classnames.tr_edit)) {
        $tr
          .removeClass(that.classnames.tr_edit)
          .find('> td.' + that.classnames.td_edit).each(function() {
            var $td = $(this); var td_index = $td.index(); var model = columnModel[td_index]; var html = $td.data(that.datanames.td_html)
      ...
      if ($tr.hasClass(that.classnames.tr_add)) {
        var isRemove = false; var changeData = $tr.data(that.datanames.changeData)
        ...
        if ($.isEmptyObject(changeData)) {
          ...
          if ($.isEmptyObject(changeData)) { isRemove = true }
        }
  ...
  Datagrid.prototype.inlineEditComplete = function($tr, trData, $btn) {
    var that = this; var columnModel = that.columnModel; var tr_index = $tr.index(); var $tds = $tr.find('> td'); var hasLinenumber = false; var $trs = that.$tbody.find('> tr'); var $addTrs = $trs.filter('.' + that.classnames.tr_add)
    ...
    $.each(columnModel, function(i, n) {
      ...
      var label = trData[n.name]; var render_label; var $td = $tds.eq(n.index)
    ...
    if ($btn && $btn.length) { $btn.trigger('bjui.datagrid.update.tr') }
  ...
  Datagrid.prototype.dialogEdit = function($tr, isAdd, data) {
    ...
    var that = this; var options = that.options; var tools = that.tools; var columnModel = that.columnModel; var tr_index = $tr.index(); var $dialog; var $form; var html = ''; var title
    ...
    var onLoad = function($dialog) {
      ...
      var $btns = $dialog.find('div.bjui-pageFooter button')

      var $prev = $btns.first()

      var $next = $btns.eq(1)

      var $cancel = $btns.eq(2)
      ...
      var createForm = function(data, $form, $tr) {
        ...
        $.each(columnModel, function(i, n) {
          ...
          var $input = $(that.inputs[i]); var $p = $('<p></p>'); var id = 'datagrid-dialog-edit-column-' + i
      ...
      $save.click(function() {
        var changeData; var data; var postData; var callback = options.editCallback
    ...
    var onClose = function() {
      that.$tbody.find('> tr.' + that.classnames.tr_add).each(function() {
        var isRemove = false; var $tr = $(this); var changeData = $tr.data(that.datanames.changeData)
  ...
  Datagrid.prototype.resizeGrid = function() {
    var that = this; var $target = that.$grid.getPageTarget(); var parentW; var parentH
    var _resizeGrid = function() {
      ...
      if (String(that.options.height).endsWith('%')) {
        ...
        if ($target.is(':hidden') && $target.hasClass('navtabPage')) {
          ...
          if (parentH != that.parentH) { that.tools.fixedH(parentH) }
        }
      }
    }
    ...
    $target.on('bjui.navtab.switch', $.proxy(function() {
      if (that.$grid.data('need.fixedH')) {
        parentH = that.$grid.parent().height()
        if (parentH) { that.tools.fixedH(parentH) }
